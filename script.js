$(document).ready(function () {
      ...
      .then(function (idMeal) {
        $.ajax({
          ...
          for (let i = 0; i < 20; i++) {
            ingredients.push(
              "<p>" +
                meal["strMeasure" + String(i + 1)] +
                "-" +
                meal["strIngredient" + String(i + 1)] +
                "</p>"
            );
          }
          ...
          for (let i = 0; i < 20; i++) {
            let ingr =
              meal["strMeasure" + String(i + 1)].trim() +
              " " +
              meal["strIngredient" + String(i + 1)];
            recipe.push(ingr);
          }
  ...
  $(".card").click((e) => {
    ...
    let nutrionURL =
      "https://api.edamam.com/api/nutrition-data?app_id=00b98447&app_key=a1e10979029b93cd4d943e72454c337f&nutrition-type=cooking&";
    $.ajax({
      url: nutrionURL + $.param({ ingr: recipeStr }),
      method: "GET",
    }).then((response) => {
      $(".nutrition").removeClass("d-none");
      $(".nutrition img").attr("src", mealImage);
      ingredients.forEach((elem) => $("#ingredients").append(elem));
      $("#nutrition-facts").append(
        "<p> Amount: " + response.calories + " calories</p>"
      );
      $("#nutrition-facts").append(
        "<p> Amount: " +
          response.totalDaily.CHOCDF.label +
          " " +
          response.totalDaily.CHOCDF.quantity +
          "</p>"
      );
